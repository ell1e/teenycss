...
int _teenycss_secrandom_GetBytesUpTo256(char *ptr, size_t amount) {
    if (amount == 0)
        return 1;
...
static void _teenycss_secrandom_initProvider() {
    if (hProvider)
        return;
    ...
    if (result == FALSE) {
        ...
        if (s)
            LocalFree(s);
        fprintf(
            stderr, "teenycss_secrandom.c: error: %s\n", buf
        );
        int msgboxID = MessageBox(
            NULL, buf, "teenycss_secrandom.c ERROR",
            MB_OK|MB_ICONSTOP
        );
        exit(1);
    }
...
__attribute__((constructor)) static void _ensureInit() {
    _teenycss_secrandom_initProvider();
}
#endif


int teenycss_secrandom_GetBytes(char *ptr, size_t amount) {
    #if !defined(_WIN32) && !defined(_WIN64)
    char *p = ptr;
    size_t fetched = 0;
    while (fetched < amount) {
        ...
        if (next_bytes > 256)
            next_bytes = 256;
        if (!_teenycss_secrandom_GetBytesUpTo256(p, next_bytes))
            return 0;
        fetched += next_bytes;
        p += next_bytes;
    }
